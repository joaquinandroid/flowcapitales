...
class TodosCModel: ViewModel() {
    //////////////////////////PERSONALIZADO//////////////////////////////
    var listado1 = GameStateProvider.todosCapitales
    var listado2 = GameStateProvider.todosPaises
    ...
    fun crearListadoParaOpciones(indice: Int): MutableList<String>{
        ...
        var opc1 = GameStateProvider.todosPaisesReferencia.get(Random.nextInt(GameStateProvider.todosPaisesReferencia.size))
        while(listadoParaOpciones.contains(opc1)){
            opc1 = GameStateProvider.todosPaisesReferencia.get(Random.nextInt(GameStateProvider.todosPaisesReferencia.size))
        }
        listadoParaOpciones.add(opc1)
        var opc2 = GameStateProvider.todosPaisesReferencia.get(Random.nextInt(GameStateProvider.todosPaisesReferencia.size))
        while(listadoParaOpciones.contains(opc2)){
            opc2 = GameStateProvider.todosPaisesReferencia.get(Random.nextInt(GameStateProvider.todosPaisesReferencia.size))
        }
